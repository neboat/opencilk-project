...
AArch64TargetLowering::EmitSetjmp(MachineInstr &MI,
  ...
  // TODO: The four stores generated by setjmp should be merged into two
  // pairs.  They are generated out of order by two separate blocks of
  // code (0+2 by machine independent code and 1+3 here).

  // Calculate resume address.  ADR has +/- 1 MB range.
  ...
  MIB = BuildMI(*thisMBB, MI, DL, TII->get(AArch64::STRXui));
  MIB.addReg(TRI->hasBasePointer(*MF) ? TRI->getBaseRegister() : AArch64::XZR);
  MIB.addReg(AddrReg);
  MIB.addImm(3); // scaled by word size
...
AArch64TargetLowering::EmitLongjmp(MachineInstr &MI,
  ...
  MIB.addReg(AArch64::FP, RegState::Define);
  MIB.addReg(PC, RegState::Define);
  MIB.addReg(AddrReg);
  MIB.addImm(0); // scaled by word size
  // SP (indirectly) and X19
  // X19 may be used as the base pointer for an over-aligned stack frame.
  // If not, the setjmp restore block does not expect values in X19 to be live.
  MIB = BuildMI(*MBB, MI, DL, TII->get(AArch64::LDPXi));
  MIB.addReg(StackReg, RegState::Define);
  MIB.addReg(TRI->getBaseRegister(), RegState::Define);
  MIB.addReg(AddrReg);
  MIB.addImm(2); // scaled by word size
